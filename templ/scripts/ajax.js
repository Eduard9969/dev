(function ($) {
    ...
    if ($.support.pjax) {
      ...
      $(document).on('click', '#subnav .navside a, #subnav .item-title a', function(event){
          event.preventDefault();
          var ident = $(this).parent().attr('data-content');
          ...
          $.ajax({
              ...
              if(data) {
                ...
                $.each(data, function(key, val) {
                  $('.test_list [data-id="'+key+'"]').html(val);
                  $('[data-model="'+ident+'"] [data-id="'+key+'"]').html(val);
                });
              }
          });
      });
