...
class Services_WTF_Test {
     ...
     * download_resources()
     *
     * Downloads test resources to a pagespeed insight
     *
     * $url      string/array        url to analysis (not empty)
     * $location string                location to download to
     * $strategy string              type of testing (if empty default values desktop )
     *
     * returns true if successful, the error message if an error occured
     */
    public function download_pagespeed($url, $location = './', $strategy = 'desktop') {

        if ( !$this->completed() )
            return false;

        // Google API KEY
        $key = 'AIzaSyDSBJK-QWQTcGkjKepW6oIceP_hjXWI6Tc';

        if($strategy != 'desktop' and $strategy != 'mobile')
            return false;

        // Google URL Analysis
        $resource = 'https://www.googleapis.com/pagespeedonline/v4/runPagespeed?url='.$url.'&locale=ru&strategy='.$strategy.'&key='.$key;

        $file = fopen( $location.'pagespeed_'.$strategy.'.txt', "w" );
    ...
    * returns true if successful, the error message if an error occured
     */
    public function download_results($location = './') {
        if ( !$this->completed() )
            return false;
    ...
    * download_results()
    *
    * Downloads test locations resources to a specified location
    *
    * $location string                location to download to
    *
    * returns true if successful, the error message if an error occured
     */
    public function download_locations($location = './') {
        if ( !$this->completed() )
            return false;

        $results = $this->locations();
        $filename = $location.'locations.txt';
